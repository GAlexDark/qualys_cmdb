 ...
 */
CREATE OR REPLACE PROCEDURE qualys.vmdr_irc_report_sbdc_only(min_value_month integer, range_interval_month integer default 0,
...
begin
    ...
    insert into report.vmdr_irc_aggregated_report (ipaddress, dns, netbios, tracking_method, os, ip_status,
        ...
        select qualys.ipadress_agg(vir.ipaddress),
        ...
        group by vir.short_name, vir.qid;

end;
$$;

alter procedure qualys.vmdr_irc_report_sbdc_only(min_value_month integer, range_interval_month integer, min_severity integer, max_severity integer, report_label varchar) owner to gsw_datasetter;
--call qualys.vmdr_irc_report_sbdc_only(range varchar, min_severity integer, max_severity integer, label varchar);