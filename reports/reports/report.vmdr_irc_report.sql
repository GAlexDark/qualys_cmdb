 ...
 */
CREATE OR REPLACE PROCEDURE qualys.vmdr_irc_report(min_value_month integer, range_interval_month integer default 0,
...
begin
    ...
    insert into report.vmdr_irc_aggregated_report (ipaddress, dns, netbios, tracking_method, os, ip_status,
        ...
        select vir.ipaddress, --qualys.ipadress_agg(vir.ipaddress),
               ...
               report_label,
               string_agg(distinct vir.short_name, ','),
               v_date_written,
        ...
        group by vir.ipaddress, vir.qid;

end;
$$;

alter procedure qualys.vmdr_irc_report(min_value_month integer, range_interval_month integer, min_severity integer, max_severity integer, report_label varchar) owner to gsw_datasetter;
--call qualys.vmdr_irc_report(range varchar, min_severity integer, max_severity integer, label varchar);